...
const Courses = () => {
    ...
    return (
        ...
        <div className="container">
            ...
            {
                    course.map((data) => (
                        <Link key={data.id} to='/' onClick={() => console.log('Hello')} className="cursor-pointer mt-10 block p-6 max-w-sm rounded-lg border shadow-md text-black bg-white hover:bg-green hover:text-white mx-auto h-28">
                            <h2 key={data.id} onClick={() => console.log('Hello')} className="mb-2 text-3xl font-normal tracking-tight">
                                {
                                    data.name
                                }
                            </h2>
